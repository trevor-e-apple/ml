...
use std::{
    ...
    vec,
    path, println, fs::File, io::Read, todo,
};
...
fn main() {
    ...
    // metal test
    {
        ...
        let mut buffer_one = device.new_buffer(array_length, MTLResourceOptions::empty());
        let mut buffer_two = device.new_buffer(array_length, MTLResourceOptions::empty());
        let mut buffer_three = device.new_buffer(array_length, MTLResourceOptions::empty());
        ...
        let thread_group_size = MTLSize { width: thread_group_len, height: 1, depth: 1 };
    ...
    {
        let k = 10;
        let library_path = path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/metal_kernels/knn.metal");
        println!("{}", library_path.as_path().display().to_string());
        ...
        let options = CompileOptions::new();
        let _library = device.new_library_with_source(&contents, &options).unwrap();
        let function = _library.get_function("predict_classes", None).unwrap();
        let pipeline = device.new_compute_pipeline_state_with_function(&function).unwrap();
        let command_queue = device.new_command_queue();
        let array_length: u64 = 8192;
        let mut buffer_one = device.new_buffer(array_length, MTLResourceOptions::empty());
        let mut buffer_two = device.new_buffer(array_length, MTLResourceOptions::empty());
        let mut results_buffer = device.new_buffer(array_length, MTLResourceOptions::empty());
        let buffer_four = device.new_buffer(k, MTLResourceOptions::StorageModePrivate);

        todo!("Fill buffer sith same data that cpu based knn test uses");
        gen_data_mtl_buff(&mut buffer_one);
        gen_data_mtl_buff(&mut buffer_two);
