...
use super::{data_structs::KnnCommon, KnnLib, Label};
...
impl KnnLib for Knn {
    fn new(k: usize, data: Vec<Vec<f32>>, labels: Vec<Label>) -> Self {
        Self { data: KnnCommon { k, data, labels } }
    }
    ...
    fn add_test(library_path: &String, num_elements: u32) {
        autoreleasepool(|| {
            ...
            let pipeline_state = create_pipeline_state(&device, library_path);
            encoder.set_compute_pipeline_state(&pipeline_state);

            let (buffer, sum) =
                create_input_and_output_buffers(&device, num_elements);
            ...
            let thread_group_count = MTLSize {
                width: ((num_elements as NSUInteger + num_threads)
                    / num_threads),
                height: 1,
                depth: 1,
            };
            ...
            encoder
                .dispatch_thread_groups(thread_group_count, thread_group_size);
...
fn handle_compute_pass_sample_buffer_attachment(
    ...
    sample_buffer_attachment_descriptor
        .set_sample_buffer(counter_sample_buffer);
...
fn handle_timestamps(
    ...
    let micros = microseconds_between_begin(
        pass_start,
        pass_end,
        gpu_time_span,
        cpu_time_span,
...
fn create_counter_sample_buffer(device: &Device) -> CounterSampleBuffer {
    let counter_sample_buffer_desc =
        metal::CounterSampleBufferDescriptor::new();
    ...
    let timestamp_counter =
        counter_sets.iter().find(|cs| cs.name() == "timestamp");

    counter_sample_buffer_desc.set_counter_set(
        timestamp_counter.expect("No timestamp counter found"),
    );
...
fn microseconds_between_begin(
    ...
    let nanoseconds =
        time_span / (gpu_time_span as f64) * (cpu_time_span as f64);
    nanoseconds / 1000.0
}
